...
class AdminTemplateManager
{
    ...
    public static function getInstence()
    {
        ...
        // 创建创造器
        $admin_creator = new AdminTemplateManager();
        $admin_creator->createPagePath = $app_creator->admin->page;
        ...
        $admin_creator->apiPrefix =  "";

        // echo "filepath: " . $admin_creator->createPagePath . "\r\n";
        // echo 'routerpath: ' . $admin_creator->createRouterPath . "\r\n";
        // echo 'apipath: ' . $admin_creator->createApiPath . "\r\n";
        // echo 'requestpath: ' . $admin_creator->createRequestPath . "\r\n";

        return $admin_creator;
    }
    ...
    public static function clear($table_array)
    {
        ...
        LocalLog::INFO("admin", "删除page 文件");
        foreach ($table_array as $key => $value) {

            $className = TableUtil::getClassName($key);

            $path = $admin_creator->createPagePath . $className;
            $dirFileCount = FileUtil::dirFileCount($path);
            if ($dirFileCount != 1) {
                LocalLog::SEPRATOR("admin", "文件超过1个，请核对后再自行删除");
            } else {
                echo "deleting " . $path . "\r\n";
        ...
        LocalLog::INFO("admin", "删除router");
        $admin_creator->createRouters([]);

        LocalLog::INFO("admin", "删除api");
        $admin_creator->createApis([]);

        LocalLog::INFO("admin", "删除api request 文件");
        $admin_creator->createRequest([]);
    }
